...
sortCarByYearAscendingly(carsDataAscending);
...
function sortCarByYearAscendingly(cars) {
  ...
  result.sort((old, young) => old.year - young.year);

  for (let i = 0; i < cars.length; i++) {
    console.log(`mobil dari indexs : ${i}, dengan tahun ${result[i].year}`);
  }

  // if(result[i].year) {
  //   result.push(cars[i].year);
  // }
